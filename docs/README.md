...
| 03 | Architecture | À rédiger | Schéma backend Django/DRF, frontend React, base PostgreSQL, modèle de données, API, déploiement. |
| 04 | Diagrammes UML | À rédiger | Cas d'utilisation, séquence (circuit de validation à 3 niveaux), classes (modèle de données), états (cycle de vie d'une demande de congé). |
